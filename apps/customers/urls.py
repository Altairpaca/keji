...
重复检测与合并 duplicates/ / merge/ 为 T4.4 追加；批量导入 import/* 为 T4.5
追加（模板下载 / 预览 / 确认，规格 §16）。
"""
...
from apps.customers.views import imports as import_views
...
urlpatterns = [
    ...
    # 批量导入（T4.5 / 规格 §16）：模板下载 / 预览 / 确认
    path("import/", import_views.import_preview, name="import_preview"),
    path(
        "import/template/",
        import_views.download_import_template,
        name="import_template",
    ),
    path(
        "import/confirm/",
        import_views.import_confirm,
        name="import_confirm",
    ),
]
